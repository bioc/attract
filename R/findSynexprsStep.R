...
calcInform <- function(exprs.dat, cl, class.vector){ 
	...
	avgmss <- mean(apply(cbind(1:k), 1, calc.onemss, exprs.dat, cl, class.vector))
	return(avgmss)
}
...
calcRss <- function(exprs.dat, cl, class.vector){ 
	...
	avgrss <- mean(apply(cbind(1:k), 1, calc.onerss, exprs.dat, cl, class.vector))
	return(avgrss)
}
...
calcModfstat <- function(exprs.dat, cl, class.vector){
	...
	avgfstat <- mean(apply(cbind(1:k), 1, calc.onefstat, exprs.dat, cl, class.vector))
	return(avgfstat) 
}
